    ...
    *,
    ...
    if exp_str:
        try:
            ...
            # MR.14 commit 4a — restored hard-coded 30-day window.
            # The MR.13 ELIGIBILITY_BYPASS_DAYS_REMAINING override is
            # gone alongside the renewal-automation pipeline.
            if days_left > 30:
                eligibility.blocking_reasons.append(
...
def create_permit_renewal_routes(
    ...
    # MR.14 commit 5 — DELETE /filing-jobs/{id} and
    # POST /filing-jobs/{id}/operator-input REMOVED.
    # Both endpoints were unreachable in the v1 monitoring
    # product (no enqueue path post-4a means no in-flight or
    # queued FilingJob to cancel; no worker means no CAPTCHA/2FA
    # challenge to respond to). Historical filing_jobs are
    # surfaced read-only via GET /filing-jobs +
    # GET /dob-confirmation; that's the entire filing_jobs API
    # surface in v1.
