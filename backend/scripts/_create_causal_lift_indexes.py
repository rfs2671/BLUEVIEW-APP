"""Phase 1 Week 13-19 PR-A — Atlas index creation for the causal_lift_matrix
collection.

Idempotent: create_index returns the existing index when an
identically-shaped one is already present. Mirrors the Phase 1 Week 3
_create_baseline_aggregates_indexes.py pattern.

Indexes:

  causal_lift_matrix
    • { complaint_bucket: 1, window_days: 1, lift_ratio: -1 }   (lookup —
      drives GET /api/causal-lift?complaint_bucket=X&window_days=W
      with default ORDER BY lift_ratio DESC)
    • { computed_at: 1 }                                         (cleanup —
      sortable for manual TTL pruning if the collection ever needs it.
      300 rows is tiny so this is more of a hygiene index than a
      performance one)

Usage:
    MONGO_URL='mongodb+srv://...' DB_NAME='blueview' \\
        python -m scripts._create_causal_lift_indexes

    # Plan-only (no writes) — prints what would be created
    python -m scripts._create_causal_lift_indexes --dry-run
...
INDEX_SPECS: List[IndexSpec] = [
    IndexSpec(
        collection="causal_lift_matrix",
        keys=[
            ("complaint_bucket", 1),
            ("window_days",      1),
            ("lift_ratio",      -1),
        ],
        name="clm_complaint_bucket_window_days_lift_ratio",
    ),
    IndexSpec(
        collection="causal_lift_matrix",
        keys=[("computed_at", 1)],
        name="clm_computed_at",
    ),
]
