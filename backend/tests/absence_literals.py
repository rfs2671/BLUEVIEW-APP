"""The scanner behind the bare-absence-literal rule, shared by two callers.

WHY THIS IS A MODULE AND NOT A TEST. The rule is enforced in two places and
must be ONE implementation: `test_absence_literals_are_specific.py` audits the
whole suite at gate time, and `scripts/check_absence_literals.py` audits the
STAGED files from `.githooks/pre-commit`. A second copy of the classifier or
of the allowlist would drift, and the drift would be silent in the direction
nobody watches — the hook passing what the gate then fails, or worse the other
way round.

WHY THE HOOK EXISTS AT ALL. The gate caught this same reflex four separate
times in one week. Each catch cost a round trip: write the assertion, push,
watch the suite fail, anchor the literal, push again. The detection was never
the missing piece; the POSITION was. Moving it in front of the commit costs
one PR and removes a recurring one.

WHAT THE HOOK CANNOT DO, stated so it is not assumed. It sees only the files
in the commit, so it cannot run `the_allowlist_does_not_rot` (an entry going
stale is visible only against a full scan) and it cannot run the floors in
`the_scanner_is_still_finding_things` (a rate needs the whole denominator).
Those stay in the test, and the test remains the gate. The hook is an early
warning on the one check that IS local to a file: a new bare literal.

Everything below this header was MOVED from the test unchanged.
...
def scan(paths: Iterable[Path] | None = None
         ) -> tuple[list[_Finding], int, int, int]:
    """(bare findings, anchored count, unclassified count, total assertNotIn).

    `paths` defaults to the whole backend suite, which is what the gate wants.
    The hook passes the staged files instead — the allowlist keys on BASENAME,
    so a staged blob written to a temp directory under its own name is scanned
    against exactly the same entries.
    """
    if paths is None:
    ...
    for path in paths:
        ...
        except (SyntaxError, OSError):  # a broken file fails elsewhere
            continue
        ...
        for node in ast.walk(tree):
            ...
            if any(c in _ANCHORS for c in literal):
                ...
                bare.append(_Finding(Path(path).name, node.lineno, literal))
    return bare, anchored, unclassified, total


def offenders(paths: Iterable[Path] | None = None) -> list[_Finding]:
    """The bare literals that are NOT justified in the allowlist."""
    bare, _a, _u, _t = scan(paths)
    return [f for f in bare if (f.file, f.literal) not in _BARE_BY_DESIGN]
...
ADVICE = (
    ...
    "\"Pass\" is in \"Password\", \"Bypass\" and a worker called Passarelli.\n"
    "Anchor the literal so it bans a CONSTRUCT instead:\n"
    "    >Pass<        render_pass_cell(        \"result\": \"Pass\"\n"
    "...or, if banning the WORD really is the claim, add it to "
    "_BARE_BY_DESIGN in backend/tests/absence_literals.py with a reason."
)
