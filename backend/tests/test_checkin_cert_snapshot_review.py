...
class CheckinHtmlOcrRetakeTest(unittest.TestCase):
    ...
    def test_manual_entry_fallback_exists(self):
        """Reconciled with the photo-optional fix: the card fields are ALWAYS
        visible rather than revealed after the retake cap, so there is no
        hidden container and no separate reveal step. The retake prompt is
        advisory and must never disable Next — goStep() is the single gate."""
        # Card fields present and NOT inside a hidden reveal container.
        self.assertIn('id="regCardNumber"', self.html)
        self.assertIn('id="regExpiration"', self.html)
        self.assertNotIn('id="ocrManualFields"', self.html)
        self.assertNotIn("function revealManualFields", self.html)
        # The bounded retake loop still exists.
        self.assertIn("ocrAttempts < OCR_MAX_ATTEMPTS", self.html)
        self.assertIn("resetCardCameraZone", self.html)
        # The retake path must not disable the Next button.
        self.assertNotIn("btnStep1Next').disabled = true", self.html)
        # goStep still accepts manual details in place of a photo.
        self.assertIn("!oshaImage && !oshaData && !hasManualCardDetails()", self.html)
