...
async def handle(payload: dict, context: HandlerContext) -> HandlerResult:
    ...
    # 2. Launch Playwright + per-GC BrowserContext.
    # MR.13 — real Chrome (channel="chrome") launched headed inside
    # the worker container's Xvfb display. See
    # docs/architecture/akamai-bypass-decision.md for the trail of
    # discarded approaches (warm cookies / fingerprint matching /
    # Bright Data which is gov-blocked / etc.). The load-bearing
    # change vs. pre-MR.12 is `channel="chrome"` plus `headless=False`
    # under Xvfb — the resulting JA3 + behavioral signature matches
    # a human's desktop Chrome.
    #
    # Optional Webshare proxy layering: if WEBSHARE_PROXY_URL is set,
    # egress goes through that residential IP instead of operator's
    # ISP. Useful only as a fallback if operator's IP starts being
    # flagged. Default v1 path is direct.
    #
    # _get_async_playwright is a module-level patchable helper so
    # tests can swap it without touching playwright itself.
    try:
    ...
    async with apw_factory() as pw:
        launch_kwargs = get_launch_args(headless=False)
        proxy_cfg = get_proxy_args()
        if proxy_cfg is not None:
            launch_kwargs["proxy"] = proxy_cfg
            logger.info(
                "[dob_now_filing] launching real Chrome (channel='chrome') "
                "with Webshare proxy fallback enabled",
            )
        else:
            logger.info(
                "[dob_now_filing] launching real Chrome (channel='chrome') "
        ...
        try:
            ...
            # MR.13 — re-engaged the per-GC storage_state machinery.
            # with_browser_context loads ~/.levelog/agent-storage/
            # <license>/current.json into the BrowserContext on entry
            # and saves it back on success (MR.11.3 preserve-on-failure
            # semantics: skipped when handler returns failed/cancelled,
            # so the seeded session survives transient run failures).
