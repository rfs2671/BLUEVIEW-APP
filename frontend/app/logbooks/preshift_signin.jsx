 ...
 * "I CHECKED THIS CARD".
 *
 * An ATTESTATION: the CP is saying he has SEEN the physical card, which is a
 * different claim from dismissing a warning. The words are ruled and are in
 * sstFlagCopy.js.
 *
...
export default function PreShiftSignIn() {
  ...
  const buildFlagMap = (checkins, flaggedItems = []) => {
    ...
    for (const c of checkins) {
      ...
      map[key] = {
        ...
        // ── What the card check is made AGAINST, and what stands.
  ...
  /**
   * Record that the CP has SEEN this worker's physical SST card.
   *
   * The only thing in the product that can lower `needs_review`. Before it, the
   ...
   * flag lives in (`workers`).
   *
   * READ THE PR BODY BEFORE SHIPPING THIS. Measured against production it
   * clears none of the 25 flagged certs, and lowering `needs_review` has a
   * second effect nobody asked for: `card_image_may_be_replaced` reads
   * `needs_review or expiration_date is None`, so six named workers would lose
   * the ability to have a wrong card photo replaced by a re-scan.
   *
  ...
  const renderWorkerFlags = (worker, index) => {
    ...
    // The card check answers "has a human looked at this card", which is only
    // the question being asked when the card itself could not be confirmed. An
    // expired card is not a reading problem and looking at it will not renew
    // it; a missing one has no card to look at.
