...
import { logbooksAPI, cpProfileAPI } from '../../src/utils/api';
...
export default function SubcontractorOrientation() {
  ...
  const [newCpSignature, setNewCpSignature] = useState(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (profile) {
        ...
        setNewCpName(profile.cp_name || '');
        if (profile.cp_signature) setNewCpSignature(profile.cp_signature);
      }
        ...
        >
          ...
          {showAddForm && (
            <GlassCard style={styles.addForm}>
              ...
              {cpProfile?.has_signature && newCpSignature && (
 ...
 */
function OrientationSignaturePanel({ cpProfile, onSign }) {
  const [cpName, setCpName] = useState(cpProfile?.cp_name || '');
  const [cpSignature, setCpSignature] = useState(
    cpProfile?.has_signature ? cpProfile.cp_signature : null
  );
  const [saving, setSaving] = useState(false);
  ...
  return (
    <View style={styles.signPanel}>
      ...
      {cpProfile?.has_signature && cpSignature && (
        <View style={styles.autoSignBadge}>
          <CheckCircle size={14} strokeWidth={1.5} color="#4ade80" />
          <Text style={styles.autoSignText}>Auto-filled from your CP profile</Text>
        </View>
      )}
      <SignaturePad
        title="Competent Person Signature"
