...
console.log('\n1. NO ID, NO SUCCESS');
{
  ...
  for (const [what, needle] of [
    ...
    ok(guardAt >= 0 && at > guardAt,
      `${what} is unreachable without an id — it comes after the refusal`);
  }
}
...
console.log('\n2. THE SEAL IS NOT OPTIONAL');
{
  ...
  ok(finalizeAt >= 0 && toastAt >= 0 && finalizeAt < toastAt,
    'and it is awaited BEFORE the screen claims the log is locked');
}
